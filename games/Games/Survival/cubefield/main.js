...
function init() {
  ...
  triGeo.vertices.push(
    ...
    new THREE.Vector3(-0.017, 0.0, 0.04),
    new THREE.Vector3(0.017, 0.0, 0.04),
    new THREE.Vector3(0.0, 0.013, 0.02)
  );
  ...
  triEdges = new THREE.LineSegments(
    edgeGeo,
...
function initStars() {
  ...
  var geo = new THREE.BoxBufferGeometry(0.5, 0.5, 0.1);
...
function gameReset() {
  ...
  console.log(ref, amount, "ref, amount");

  const secretKey = "HDk2PdH6jmcqtaXP69UiPEuupK5FrdIV5+QyKLng+L0=";
  ...
  var ciphertext = CryptoJS.AES.encrypt(msg, secretKey).toString();

  console.log(ciphertext, "ciphertext");

  const decryptedGameId = CryptoJS.AES.decrypt(ref, secretKey).toString(
    CryptoJS.enc.Utf8
  );

  console.log(decryptedGameId, "decryptedGameId");
...
function animate() {
  ...
  switch (phase) {
    ...
    case 1:
      ...
      // Level speed
      var levelSpeed =
        (1 + level * 0.5) * rate * 0.5 * (1 + stnDiff * 1.5) + zSpeed + 0.05;
      ...
      triangle.position.z =
        triEdges.position.z =
        plane.position.z =
          camera.position.z + (stnCam == 0 ? -2.2 : stnCam == 1 ? -1.2 : 0.2);
      triangle.position.x =
        triEdges.position.x =
        plane.position.x =
          camera.position.x;
...
function updateDiff(target) {
  ...
  for (var k = 0; k < 3; k++) opt[k].classList.remove("focus");
