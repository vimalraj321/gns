...
function init() {
  (width = window.innerWidth),
    ...
    (xSpeed = 0),
    (zSpeed = 0),
    (leftArrow = rightArrow = upArrow = !1),
    (mCubes = []),
    (starSpeed = []),
    (stars = []),
    ((scene = new THREE.Scene()).fog = new THREE.Fog(10724259, 5, 25)),
  ...
  triGeo.vertices.push(
    ...
    new THREE.Vector3(-0.017, 0, 0.04),
    new THREE.Vector3(0.017, 0, 0.04),
    new THREE.Vector3(0, 0.013, 0.02)
  ),
    ...
    ((triangle = new THREE.Mesh(
      triGeo,
      new THREE.MeshBasicMaterial({ color: 6579300 })
    )).castShadow = !0),
  ...
  (triEdges = new THREE.LineSegments(
    ...
    scene.add(triEdges),
    (spotlight = new THREE.SpotLight(16777215)).position.set(0, 100, 0),
...
function initStars() {
  ...
  for (
    var texture = new THREE.TextureLoader().load("assets/images/star.png"),
      ...
      geo = new THREE.BoxBufferGeometry(0.5, 0.5, 0.1),
      i = 0;
    i < 10;
    i++
  )
    (starSpeed[i] = []),
      ...
      (stars[i].position.x = 20 * Math.random() - 10),
      (stars[i].position.y = 10 * Math.random()),
      (stars[i].rotation.z = 2 * Math.random() * Math.PI),
      (starSpeed[i][0] = 0.003 * Math.random() - 0.0015),
      (starSpeed[i][1] = 0.003 * Math.random() - 0.0015),
      scene.add(stars[i]);
}
function gameReset() {
  for (var c of ((uiNewGame.style.visibility = "hidden"),
...
function setPreGame() {
  for (var c of ((uiScore.style.color = "black"),
  (document.body.style.backgroundColor = "#A3A3A3"),
  ...
  triangle.material.color.setHex(6579300),
  triEdges.material.color.setHex(0),
  mCubes))
    c.updateDesign(0);
  for (var s of ((level = 0),
  plane.material.color.setHex(13158600),
  scene.fog.color.setHex(10724259),
  stars))
    s.position.z = 5;
}
function newLevel() {
  for (var c of ((levelBreak = new Date().getTime()), level++, mCubes))
  ...
  switch (level % 5) {
    case 0:
      ...
      break;
    case 1:
      for (var s of (plane.material.color.setHex(0),
      scene.fog.color.setHex(0),
      triangle.material.color.setHex(0),
      triEdges.material.color.setHex(65280),
      (document.body.style.backgroundColor = "#000000"),
      stars))
        s.position.z = -50;
      break;
    case 2:
      for (var s of (plane.material.color.setHex(9868950),
      scene.fog.color.setHex(9868950),
      triangle.material.color.setHex(0),
      triEdges.material.color.setHex(0),
    ...
    case 3:
      plane.material.color.setHex(16777215),
        scene.fog.color.setHex(16777215),
        triangle.material.color.setHex(6579300),
        triEdges.material.color.setHex(0),
        (document.body.style.backgroundColor = "#FFFFFF");
      break;
    case 4:
      plane.material.color.setHex(9868950),
        scene.fog.color.setHex(9868950),
        triangle.material.color.setHex(16724588),
        triEdges.material.color.setHex(16777215),
        (document.body.style.backgroundColor = "#969696");
  }
}
function animate() {
  ...
  switch (phase) {
    case -1:
      (xSpeed *= 0.95), (camOffset *= 0.95);
      var levelSpeed = (level + 1) * rate * 0.5 + (zSpeed *= 0.95);
      (camera.rotation.z = 0.7 * (camera.rotation.z + 1.5 * xSpeed)),
        ...
        updateCubes(0.2 * rate, levelSpeed);
      break;
    case 0:
      break;
    case 1:
      rightArrow || leftArrow
        ? (leftArrow && (xSpeed += 0.02 * rate),
          rightArrow && (xSpeed -= 0.02 * rate))
        : (xSpeed *= 0.8),
        (xSpeed = Math.max(-0.5, Math.min(0.5, xSpeed))),
        (zSpeed = upArrow ? zSpeed + 0.025 * rate : 0.95 * zSpeed),
        (zSpeed = Math.min(0.4, zSpeed));
      levelSpeed =
        (1 + 0.5 * level) * rate * 0.5 * (1 + 1.5 * stnDiff) + zSpeed + 0.05;
      (camera.rotation.z = 0.7 * (camera.rotation.z + 1.5 * xSpeed)),
        (camera.rotation.z = Math.min(0.1, Math.max(-0.1, camera.rotation.z))),
        (camera.position.x += xSpeed * rate * -10),
        (camera.position.z -= levelSpeed),
        ...
        (triangle.rotation.y = camera.rotation.z),
        (triEdges.rotation.y = camera.rotation.z),
        (triangle.rotation.z = 2 * camera.rotation.z),
        (triEdges.rotation.z = 2 * camera.rotation.z),
        (triangle.position.y = 0.01 - Math.abs(0.05 * camera.rotation.z)),
        (triEdges.position.y = 0.01 - Math.abs(0.05 * camera.rotation.z)),
        spotlight.position.set(
          triangle.position.x,
          triangle.position.y + 10,
          triangle.position.z
        ...
        3e4 * (level + 1) < totalElapse && newLevel(),
        -1 != levelBreak && now - levelBreak > 5e3 && (levelBreak = -1),
        updateCubes(rate, levelSpeed);
  }
  level % 5 == 1 && updateStars(),
    1 == phase
      ? renderer.setClearColor(16777215, 0)
      : renderer.setClearColor(10724259, 1),
...
function updateStars() {
  for (var s = 0; s < stars.length; s++)
    (stars[s].position.x < -10 ||
      ...
      ((stars[s].position.x = 20 * Math.random() - 10),
      (stars[s].position.y = 10 * Math.random())),
...
function updateCubes(rate, levelSpeed) {
  for (var c of mCubes) {
    ...
    if (1 == phase && collision)
      return logHighscore(), (phase = -1), void setPreGame();
  }
}
function setCam() {
  switch (stnCam) {
...
function pauseGameScreen() {
  0 == phase
    ? ((phase = 1),
      ...
      (uiPause.style.opacity = 0))
    : 1 == phase &&
      ((phase = 0),
      (uiPause.style.visibility = "visible"),
      (uiPause.style.opacity = 1));
}
function returnToMenu() {
  (uiPause.style.opacity = 0),
    (uiPause.style.visibility = "hidden"),
    logHighscore(),
    (rotOffset = new Date().getTime() % 5e4),
    (phase = -1),
    setPreGame();
}
...
function updateBlock(target) {
  ...
  for (
    ...
    opt[k].classList.remove("focus");
  for (var c of (opt[target].classList.add("focus"), mCubes))
    c.reset(stnBlock, !1, stnDiff);
  document.getElementById("bounce").style.opacity = target > 0 ? "1.0" : "0.0";
}
function logHighscore() {
  totalElapse > parseInt(getCookie()) || "" == getCookie()
...
init(),
  ...
  document.addEventListener("resize", (e) => {
    (width = window.innerWidth),
      (height = window.innerHeight),
      renderer.setSize(width, height),
      (camera = new THREE.PerspectiveCamera(15, width / height, 0.1, 1e3)),
      setCam();
  }),
  document.addEventListener("keydown", (e) => {
    if (null == e.keyCode)
      e.pageX > 0.5 * width ? (rightArrow = !0) : (leftArrow = !0);
    else {
