...
export class UserController {
  ...
  userRegister = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      // =================== VALIDATING PASSWORD =========================== //
      if (!validator.isEmail(userRegisterDto.email))
        return next(new AppError("Invalid email address", 400));
      if (!validator.isStrongPassword(userRegisterDto.password))
        return next(
          new AppError(
      ...
      if (newUser.referredBy)
        await this.userService.updateEligibilityLevel([newUser.referredBy.id]);
      // ==================== SEND WELCOME EMAIL ==================== //
      ...
      return res
        .status(201)
        .json({ status: "User created successfully", token: authToken });
    }
  );
  ...
  userLogin = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (user.status === "blocked")
        return next(
          new AppError("Your account is blocked\nplease contact support", 401)
        );
  ...
  withdrawal = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!amount || amount < 10)
        return next(
          new AppError(
            "Minimum withrawal amount us $10, please enter a valid amount",
            400
          )
        );
      if (user.balance < 10 || amount > user.balance)
      ...
      res
        .status(200)
        .json({ message: `Withdarawal of ${amount} is processing.` });
    }
  );

  withdrawalHistory = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const theUser = req.user;
      if (!theUser) return next(new AppError("Unauthorized request", 400));
      const user = await this.userRepository.findOne({
  ...
  swap = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (to === "USDT") {
      ...
      user.gptBalance = parseFloat(
        (Number(user.gptBalance) - Number(amount)).toFixed(4)
      );
      user.balance = parseFloat(
        (Number(user.balance) + Number(convertedAmount)).toFixed(4)
      );
      ...
      user.balance = parseFloat(
        (Number(user.balance) - Number(amount)).toFixed(4)
      );
      user.gptBalance = parseFloat(
        (Number(user.gptBalance) + Number(convertedAmount)).toFixed(4)
      );
    } else {
  ...
  // =========================== WITHDRAWAL ============================== //
  approveWithdrawal = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const { withdrawalId } = req.body;
      if (!withdrawalId)
        return next(
          new AppError(
            "Withdrawal ID is required to perform this operation",
            400
          )
        );
      const theUser = req.user;
      ...
      if (!theWithdrawal)
        return next(
          new AppError(
            "Invalid request, please reload and check withdrawal data again",
            404
          )
        );
      if (
        theWithdrawal.status === "completed" ||
        theWithdrawal.status === "failed"
      )
        return next(
          new AppError(
            "This withdrawal is already completed, please try another",
            400
          )
        );

      // ======================== SIGN WITHDRAWAL AND SEND TO BLOCKCHAIN ================================ //
      const gatewayResponse = await this.userService.sendCoinThroughGateway(
        theWithdrawal.amount,
        theWithdrawal.user.wallet
      );
      console.log("Gate way response");
      if (gatewayResponse.status !== "OK")
  ...
  toggleUserBlock = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const { userId } = req.body;
      if (!userId)
        return next(
          new AppError("User ID is required to perform this operation", 400)
        );
      const theUser = req.user;
      const user = await this.userRepository.findOne({
        where: { email: theUser.email },
      ...
      if (!theUserToBlock)
        return next(
          new AppError(
            "Invalid request, please reload and check user data again",
            404
          )
        );

      theUserToBlock.status == "active"
        ? (theUserToBlock.status = "blocked")
        : (theUserToBlock.status = "active");
  ...
  // ======================= GET ALL WITHDRAWAL ======================== //
  getAllWithdrawalAdmin = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const theUser = req.user;
      if (!theUser) return next(new AppError("Unauthorized request", 400));
      const user = await this.userRepository.findOne({
